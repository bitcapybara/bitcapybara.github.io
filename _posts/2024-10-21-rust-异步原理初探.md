...
对于需要在后台运行的任务，如 tokio::spawn 启动的 Future，简单的方法就是创建一个队列，然后在一个循环里，不停的从队列上取下一个 future 任务，调用 poll 方法，如果返回 pending 则把 future 放到队列尾部等待下次 poll，如果返回 ready 代表任务结束，就不再放回队列，但是 poll 方法不会阻塞，这样无疑会造成 CPU 时间的大量浪费，这个时候 waker 就派上用场了。

smol 是一个精简的异步运行时，我们可以参考它的实现看任务是怎么在运行时中被推进的。smol 运行时代码分为两个主要结构，一个是 Executor 执行器，一个是 Reactor 反应器。Reactor 包含了一个 Poller，它对操作系统的异步接口入 epoll，kqueue 等进行了封装，主要做两件事情，一个是管理 IO 时间，一个是管理 timer 定时器。当一个任务是 IO 任务时，Reactor 会把这个 IO 事件（如文件读写）注册到 Poller 上，交由操作系统的事件通知机制管理，当文件可读/可写时，操作系统通知 Reactor，Reactor 调用对应 Future 的 waker 的 wake() 方法，wake 是由运行时实现的，它会通知 Executor 来对 future 进行 poll。
